    ...
    if mode=="test":
        ...
        for token in tokens:
            if token!="[PAD]":
                wf.write(token+'\n')
        wf.close()
def write_tokens_list(tokens,mode,token_test):
    print('write_tokens_list:' , tokens, mode)
    if mode=="test":
        for token in tokens:
            if token!="[PAD]":
                token_test.append(token)

def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer,mode):   ###modify:add a parameter token_test
    label_map = {}
...
def convert_single_example_test(ex_index, example, label_list, max_seq_length, tokenizer, token_test,mode
                           ):  ###modify:add a parameter token_test
    label_map = {}
    for i, label in enumerate(label_list):
        label_map[label] = i
    with open(os.path.join(FLAGS.output_dir, 'label2id.pkl'), 'wb') as w:
    ...
    # write_tokens(ntokens,mode)
    print('mode: ', mode)
    write_tokens_list(ntokens, mode, token_test)
    return feature
...
def filed_based_convert_examples_to_features(
    ...
    #writer = tf.python_io.TFRecordWriter(output_file)  ###modify
    for (ex_index, example) in enumerate(examples):
        ...
        #writer.write(tf_example.SerializeToString())  ###modify
def filed_based_convert_examples_to_features_test(
        examples, label_list, max_seq_length, tokenizer, output_file,token_test,mode=None
):
    #writer = tf.python_io.TFRecordWriter(output_file)  ###modify
    print('filed_based_convert_examples_to_features_test_mode:', mode)
    for (ex_index, example) in enumerate(examples):
        if ex_index % 5000 == 0:
            tf.logging.info("Writing example %d of %d" % (ex_index, len(examples)))
        feature = convert_single_example_test(ex_index, example, label_list, max_seq_length, tokenizer,token_test,mode)
        ...
        tf_example = tf.train.Example(features=tf.train.Features(feature=features))
        #writer.write(tf_example.SerializeToString())  ###modify
def ram_based_convert_examples_to_features(
        examples, label_list, max_seq_length, tokenizer, output_file,mode=None
):
    tf_examples = []
    for (ex_index, example) in enumerate(examples):
...
def ram_based_input_fn_builder(tf_examples, seq_length, is_training, drop_remainder):
    name_to_features = {
        "input_ids": tf.FixedLenFeature([seq_length], tf.int64),
        "input_mask": tf.FixedLenFeature([seq_length], tf.int64),
        "segment_ids": tf.FixedLenFeature([seq_length], tf.int64),
...
def create_model(bert_config, is_training, input_ids, input_mask,
    ...
    with tf.variable_scope("loss"):
        ...
        return (loss, per_example_loss, logits, predict,accuracy)
        ##########################################################################
...
class BertNer(object):
    ...
    def __init__(self):
        ...
        input = 'cancer can not be treated by dienogest.'
        tokens = word_tokenize(input)
        label_list = ['O' for i in range(len(tokens))]
        tokenized_sent = " ".join(tokens)
        tokenized_label = " ".join(label_list)
        ...
        label = tokenization.convert_to_unicode(tokenized_label)
        predict_examples.append(InputExample(guid=0, text=text, label=label))

        predict_file = os.path.join(FLAGS.output_dir, "predict.tf_record")
        filed_based_convert_examples_to_features_test(predict_examples, label_list,
                                                 FLAGS.max_seq_length, tokenizer,
                                                 predict_file,token_test, mode="test")
        ...
        print("predLabelSent:",predLabelSent)
        pred_lab=[]
        ...
        buf = []
        for t, l in zip(token_test, pred_lab):
...
def main(_):
    ...
    if FLAGS.do_train:
        ...
        print("size of train_examples",len(train_examples))
        num_train_steps = int(len(train_examples) * FLAGS.num_train_epochs / FLAGS.train_batch_size)
    ...
    if FLAGS.do_train:
        ...
        tf.logging.error("  Num examples = %d", len(train_examples))
        tf.logging.error("  Batch size = %d", FLAGS.train_batch_size)
        tf.logging.error("  Num steps = %d", num_train_steps)
