...
class GraphVisualization():
        ...
        if isinstance(node, list):
            ...
            for n in node:
                if n in self.G:
                    ...
                    nodes.update(nx.ancestors(self.G, n))
                else:
                    print("there are no adjacent nodes to the node " + "'"+ n+"'" )
                    continue
        else:
            if node in self.G:
                print("the adjacent nodes to the node " + "'"+node+"'" +":" + str(list(self.G.neighbors(node))))
                nodes.add(node)
                nodes.update([i for i in self.G.neighbors(node)])
                nodes.update(nx.ancestors(self.G, node))
        if len(nodes)==1:
    ...
    def show_gragh_by_edge(self,relation_name):
        subgraph = nx.DiGraph(((u, v, e) for u, v, e in self.G.edges(data=True) if e['relation'] == relation_name))

        # graph = nx.from_pandas_edgelist(self.kg_df[self.kg_df['edge'] == relation_name], "source", "target",
        #                             "edge", create_using=nx.DiGraph())

        plt.figure(figsize=(20, 20))
        pos = nx.spring_layout(subgraph)  # k regulates the distance between nodes
        #edge_labels = nx.get_edge_attributes(subgraph, 'edge')
        nx.draw(subgraph, with_labels=True,node_size=400,node_color='skyblue', edge_cmap=plt.cm.Blues, pos=self.pos)
