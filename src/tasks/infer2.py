...
class infer_from_trained(object):
    ...
    def get_annotated_sents_for_test(self, annotated_sent, type1, type2):
        ...
        match2 = re.search('\[/E2\]', annotated_sent)
        end2 = match2.start()
        annotated_for_test = annotated_sent[:start1] + type1 + annotated_sent[end1:start2] + type2 + annotated_sent[
                                                                                                     end2:]
    ...
    def infer_sentence(self, sentence, detect_entities=False):
        if detect_entities:
            ...
            ner_sentence = ner_lib.get_annotated_sents(sentence)
            print('ner_sentence: ', ner_sentence)
